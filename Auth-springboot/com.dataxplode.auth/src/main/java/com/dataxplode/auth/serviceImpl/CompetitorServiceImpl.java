...
public class CompetitorServiceImpl implements CompetitorStrategySearchService {
        ...
        try{
            ...
            if (validateRequest(requestMap)) {
                ...
                for (PlanFeatureTable planFeatureTable : plan.getPlanFeature()) {
                    ...
                    if ("Competitive Strategy Search".equals(featureName)) { // Use equals() for string comparison
                        featureExists = true;
                        break;
                    }
                }
                ...
                if(featureExists){
                    ...
                    //DistributorQuery
                    String competitiveSearchQuery= requestMap.get("competitiveSearchQuery");
                    //Country
    ...
    public ResponseEntity<String> getCompetitive(Integer UserID, String country, String competitive) {
        try {
            Optional<String> searchData = userSubscriptionDao.findSearchDataByUserAndCountryAndCompetitiveStratergy
                    (
                    Long.valueOf(UserID), country, competitive);

            if (searchData.isPresent()) {
                log.info("Search Data: {}", searchData.get());
                return new ResponseEntity<>(searchData.get(), HttpStatus.OK);
            } else {
                return Utils.getResponseEntity("No data found for " + competitive, HttpStatus.NOT_FOUND);
            }
        } catch (Exception ex) {
            log.error("Error occurred while retrieving Competitve data", ex);
        }
    ...
    @Override
    public ResponseEntity<UserSubscription> deleteCompetitor(Map<String, String> requestMap) {
        return null;
    }
    ...
    private boolean validateRequest(Map<String, String> requestMap) {
        ...
        List<String> requiredKeys = Arrays.asList(
                ...
                "competitiveSearchQuery",  // Search query text
